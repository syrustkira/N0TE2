...
class CapabilityNegotiator:
    ...
    freshness stay separate. The result may expose an existing CapabilityCandidate
    only after the exact case is legitimately negotiable. Ranking remains owned by
    CapabilityResolver, and execution authority remains owned elsewhere.
    """
        ...
        *,
        ...
        if not access_required:
            ...
            if characterization.entitlement_required:
                if access.entitlement_state == "DENIED":
                    reasons.append("ENTITLEMENT_DENIED")
                    hard_unavailable = True
                elif access.entitlement_state == "UNKNOWN":
                    reasons.append("ENTITLEMENT_UNKNOWN")
                    unknown = True
                elif access.entitlement_state == "NOT_REQUIRED":
                    reasons.append("ENTITLEMENT_REQUIREMENT_CONFLICT")
                    conflict = True
                else:
                    reasons.append("ENTITLEMENT_GRANTED")

            if characterization.permission_required:
                if access.permission_state == "DENIED":
                    reasons.append("PERMISSION_DENIED")
                    hard_unavailable = True
                elif access.permission_state == "UNKNOWN":
                    reasons.append("PERMISSION_UNKNOWN")
                    unknown = True
                elif access.permission_state == "NOT_REQUIRED":
                    reasons.append("PERMISSION_REQUIREMENT_CONFLICT")
                    conflict = True
                else:
                    reasons.append("PERMISSION_GRANTED")
