...
class AttentionDeferralMemory:
    """Durable Not Now state owned by the canonical Attention domain.

    This is not a task list or recommendation store. It records only the artist's
    suppression instruction for a stable N0TE item key. The item itself remains
    owned by its canonical product surface.
    """
    ...
    def _clean_item_key(value: str) -> str:
        ...
        if not item or len(item) > 200:
            raise ValidationError("item_key must contain 1..200 characters")
        return item
        ...
        try:
            ...
            for row in self._conn.execute(
                ...
                if item.horizon == "LATER_THIS_SONG" and (
                    ...
                    raise LineageCorruptionError(
                        "Later-this-Song Deferral requires Song and anchor"
                    )
    ...
    def _validate_binding(
        ...
        if horizon == "LATER_THIS_SONG":
            if not song or not anchored:
                raise ValidationError("Later this Song requires current Song and item anchor")
    ...
    def defer(
        ...
        *,
        ...
        song_id, anchor = self._validate_binding(horizon, song_id, anchor)
