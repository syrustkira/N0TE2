...
def test_consumer_capture_is_user_declared_server_owned_and_song_bound(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert observation.song_id == song.id
        assert observation.session_id == session.id
        assert observation.episode_id == episode.id
        assert observation.source_kind == "USER_DECLARED"
        assert observation.source_ref.startswith("consumer-friction:")
        assert observation.confidence == 0.7
        assert hq.friction.recurring_patterns(song_id=song.id) == ()
        events = [event.event_type for event in hq.activity.for_song(song.id)]
        assert events.count("FRICTION_OBSERVED") == 1
    finally:
        hq.close()


def test_recurrence_requires_same_explicit_key_across_distinct_sessions(tmp_path: Path) -> None:
    hq = HeadquartersMemory.create(tmp_path, "Artist")
    try:
        song = hq.store.create_song("Song")
        ...
        assert service.recurring_for_active_song() == ()
        service.record(
        ...
        assert patterns[0].key == "plugin-browsing"
        assert patterns[0].session_count == 2
        assert patterns[0].occurrence_count == 2
        assert patterns[0].prevention_hints == ("Choose three tools before the Session",)
    finally:
        hq.close()


def test_two_learning_episodes_in_one_session_do_not_manufacture_recurrence(tmp_path: Path) -> None:
    hq = HeadquartersMemory.create(tmp_path, "Artist")
    try:
        song = hq.store.create_song("Song")
...
def test_stale_active_song_fails_closed_without_write(tmp_path: Path) -> None:
    hq = HeadquartersMemory.create(tmp_path, "Artist")
    try:
        ...
        before = hq.store._conn.execute("SELECT COUNT(*) FROM friction_observations").fetchone()[0]
        with pytest.raises(StaleFrictionJourneyError):
            service.record(
                binding,
                friction_key="context-switching",
        ...
        after = hq.store._conn.execute("SELECT COUNT(*) FROM friction_observations").fetchone()[0]
        assert after == before
    finally:
        hq.close()


def test_projection_fails_closed_on_unknown_source_semantics() -> None:
    ...
    with pytest.raises(FrictionJourneyError):
